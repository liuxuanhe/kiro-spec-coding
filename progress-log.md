...
[2026-03-15 15:00] TASK-20.1 - DONE - 创建 admin-portal 项目骨架，实现管理员登录页面，支持首次登录强制修改密码、密码强度验证、路由守卫

[2026-03-15 15:15] TASK-20.2 - DONE - 实现业主审核页面，支持单个审核和批量审核，包含状态标签页筛选、驳回原因对话框、手机号脱敏显示、侧边栏菜单

[2026-03-15 14:30] TASK-20.3 - DONE - 实现车辆管理页面，支持筛选和查看详情

[2026-03-15 14:30] TASK-20.4 - DONE - 实现 Visitor 审批页面，支持单个审批和批量审批

[2026-03-15 14:30] TASK-20.5 - DONE - 实现车位配置页面，支持查看和修改车位配置

[2026-03-15 16:00] TASK-20.6 - DONE - 实现报表页面，包含入场趋势折线图、车位使用率柱状图、峰值时段柱状图，支持时间范围选择和粒度切换，添加 ECharts 5.x 依赖

[2026-03-15 17:00] TASK-20.7 - DONE - 实现僵尸车辆处理页面，支持状态筛选和处理操作

[2026-03-15 18:00] TASK-20.8 - DONE - 实现审计日志查询页面，包含操作日志和访问日志两个 Tab，支持筛选、展开详情和导出功能

[2026-03-15 17:30] TASK-20.8 - DONE - 实现审计日志查询页面，包含操作日志和访问日志两个 Tab，操作日志支持按操作类型/操作人/时间范围筛选及展开查看 before/after 值对比，访问日志支持按请求路径/IP/时间范围筛选，支持导出审计日志

[2026-03-15 17:30] TASK-20 - DONE - 管理后台页面实现全部完成（20.1~20.8），包含登录页、业主审核、车辆管理、Visitor 审批、车位配置、报表分析、僵尸车辆处理、审计日志查询共8个页面

[2026-03-15 10:00] TASK-25.1 - DONE - 编写部署文档：新增 Dockerfile、docker-compose.yml、.dockerignore、.env.example 和 docs/deployment.md，涵盖容器化部署、手动部署、环境变量、数据库初始化、生产环境注意事项及前端部署说明。

[2026-03-15 10:15] TASK-25.2 - DONE - 编写 API 接口文档：新增 docs/api.md，涵盖全部 13 个模块的接口说明、请求示例、权限要求、错误码对照表及认证鉴权说明。

[2026-03-15 10:30] TASK-25.3 - DONE - 编写运维文档：新增 docs/operations.md，涵盖监控配置、日志查询、备份恢复、故障排查及数据一致性检查。

[2026-03-15 10:45] TASK-25.4 - DONE - 编写用户手册：新增 docs/user-guide-owner.md（业主小程序手册）和 docs/user-guide-admin.md（管理后台手册），涵盖全部功能操作说明和常见问题。
[2026-03-15 10:45] TASK-25 - DONE - 部署与文档模块全部完成：包含部署文档、API 文档、运维文档、用户手册共 6 个文件。
